...
fn parse_share_url(raw: &str) -> Result<ParsedShareUrl, CoreError> {
    ...
    shr_crypto::ShareId::parse(&share_id).map_err(|_| CoreError::Usage("invalid share id".into()))?;
...
fn map_hosted(err: shr_hosted::HostedError) -> CoreError {
    match err {
        shr_hosted::HostedError::Unreachable | shr_hosted::HostedError::Network(_) => {
            CoreError::NetworkUnavailable
        }
        shr_hosted::HostedError::Api { status, .. } if status == 403 || status == 401 => {
            CoreError::AuthFailed
        }
        shr_hosted::HostedError::Api { status, .. } if status == 410 => CoreError::ShareExpired,
...
mod tests {
    ...
    fn parses_stored_url_path() {
        let parsed = parse_share_url("https://shr.rip/s/gS8M5b").unwrap();
